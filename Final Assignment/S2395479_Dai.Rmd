...
```{r label = setup, echo = F, message = FALSE, warning = FALSE}
...
Ans: 
- When K is small, there are not many color options. The RGB platte can only provide rough approximation of the original colors. This leads to high *bias*. Therefore, both permutation and original picture will have large but close log loss. Close log loss will give a small GAP statistic.
- However, too many color options will cuase high *variancce* since a tiny change in the original color may lead to a different RGB platte color. Therefore, both permutation and original picture will have small but close log loss. Close log loss will give a small GAP statistic.

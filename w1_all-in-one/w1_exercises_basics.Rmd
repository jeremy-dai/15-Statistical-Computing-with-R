...
Solution: create a new R script
...
IsMeanBiggerThanHalfway <- function(my_vector){
  halfway <- (min(my_vector)+max(my_vector))/2
  mean_value <- mean(my_vector)
  return(mean_value >halfway)
}
...
c<-2^seq(1:9)
d<-rep(0:2,6)
e<-rep(1:4,times=c(1:4))
f<-c(1,2,5)* rep(10^(1:3),each=3)
```
...
c <- exp(0:10)-3*(0:10)
d <- (3*(0:10))%%7
ApproxPi <- function(n=100){ return (4*sum(1/(2*(1:n)-1) * c(1,-1)))}
```
...
```{r}
AbsoluteDifference <- function(a=1,b=1){
  if ((a - b) < 0) {sig = -1} else {sig = 1}
  return (sig*(a-b))
}
```
...
```{r}
short_alphabet[c(rep(FALSE,6), TRUE)]
```
...
```{r}
short_alphabet[as.logical(seventh_letter +  third_letter)]
```

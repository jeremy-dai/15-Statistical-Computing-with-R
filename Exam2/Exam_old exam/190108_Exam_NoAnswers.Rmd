...
<!-- 4. Create a new variable that expresses the selling price of the houses in kilo dollars. -->

```{r}
ames_prep_2 <- ames_raw  %>% #[ames_raw$"Gr Liv Area" <= 4000,] %>%
  filter( `Gr Liv Area` <= 4000) %>%
  select("SalePrice", "Lot Area", "Total Bsmt SF" ,"Gr Liv Area", "Garage Cars" ,"Fireplaces") %>%
  na.omit() 
...
```{r}
exp.weight <- function(y,lmda) { exp(y/lmda) / sum(exp(y/lmda))}
```

```{r}
exp.weight(ames_prep$SalePrice,1)[1:5]
summary(exp.weight(ames_prep$SalePrice,1000))
```
...
```{r}
exp.weight2 <- function(y,lmda) { 
  C <- max(y/lmda)
  return(exp(y/lmda- C) / sum(exp(y/lmda- C)))
}

summary(exp.weight2(ames_prep$SalePrice,1))
```
...
```{r}
max_lmda <- colSums(ames_prep$SalePrice * v_from_0_to_1e6)
```

```{r}
y <- ames_prep$SalePrice
plot(names(v_from_0_to_1e6),max_lmda,type='l')
abline(h = max(y,na.rm = T),lty=2,col='red')
abline(h = mean(ycc,na.rm = T),lty=2,col='blue')
```
...
```{r}
lm_ols2 <- lm(SalePrice ~ `Lot Area` + `Total Bsmt SF` + 
    `Gr Liv Area` + `Garage Cars` + `Fireplaces`, data = ames_prep)
```

```{r}
all.equal(lm_ols2,lm_ols)
```
...
```{r}
par(mfrow = c(2,3))
plot(lm_ols$fitted.values,ames_prep$SalePrice,pch='.',main = "Ordinary Least Squares",)
plot(ames_prep$SalePrice,lm_ols$residuals,pch='.')
plot(lm_ols$fitted.values,lm_ols$residuals,pch='.')

plot(lm_wls$fitted.values,ames_prep$SalePrice)
plot(ames_prep$SalePrice,lm_wls$residuals)
plot(lm_ols$fitted.values,lm_wls$residuals)
...
```{r}
N=50
mean(1:50 %in% sample(N,replace = T))
```
...
```
```{r}
InSampleProb <- function(N) { 1 - (1 - 1 / N )^(N)}
InSampleProb(N)
```
...
for(i in 1:B){
  r <- nrow(ames_prep)
  ind <- sample(r, replace = T)
  df.sample <- ames_prep[ind,]
  y.train<- df.sample$SalePrice
  ols.temp <- lm(SalePrice ~ `Lot Area` + 
                 `Total Bsmt SF` + `Gr Liv Area` + `Garage Cars` + Fireplaces,
               data = df.sample)
  wls.temp <- lm(formula = SalePrice ~ `Lot Area` +
                   `Total Bsmt SF` + `Gr Liv Area` + `Garage Cars` + Fireplaces, 
    data = df.sample, weights = exp.weight2(df.sample$SalePrice,150000))
  err[i] <- cor(y.train,ols.temp$fitted.values) - cor(y.train,wls.temp$fitted.values)
  
  df.test <- ames_prep[-ind,]
  y.test <- df.test$SalePrice
  X.test <- subset(df.test,select = -SalePrice)
  y.predict.ols <- predict(ols.temp,X.test)
  y.predict.wls <- predict(wls.temp,X.test)
  err_test[i] <- cor(y.test,y.predict.ols) - cor(y.test,y.predict.wls)
}
remove(i)
increased_training_error <- err_train/B
increased_test_error <- err_test/B
```

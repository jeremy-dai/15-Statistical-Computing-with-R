...
The warning is given by the fact that we try to replace 1 list element, with a vector of numbers (or letters). What the code probably meant to do, was replace the *contents* of the list element with a vector of numbers (or letters).
E.g.: `mydlist[[1]]] <- 97:122` will work (and is probably what is meant to happen).
...
```
No, does not work, `R` does not know how 
...
**Answer:**
```{r}
head(as.data.frame(my_list[-5]))
```
Although this works, `R` has to repeat the element `name` to fit it into shape of a `data.frame`.
...
my_data_frame <- data.frame(
  norm_vals = my_list$normal_values, 
  samples = my_list[[3]], 
  id = my_list[["id"]]
)
```
...
**Answer:**
```{r}
nonrecurs_unl <- unlist(my_list, recursive = FALSE)
str(nonrecurs_unl)
```
...
**Answer:**
```{r}
recurs_unl <- unlist(my_list, recursive = TRUE)
str(recurs_unl)
```
...
```{r}
GiveBivariateSummary <- function(var1, var2){
  my_normal_matrix <- cbind(var1, var2)
  my_list <- list(
    data_matrix_example = head(my_normal_matrix, 10),
    cor = cor(my_normal_matrix),
...
GiveBivariateSummary(var1, var2)
```


### f.

Instead of returning a `list` variable / object, do you think there is another way to all of the information you've computed in your bivariate summary function?

**Answer:**

It would not be surprising if there exists a similar function in some `R` package elsewere. Probably it will be difficult to do this as neatly in anything else such as a vector, or data.frame.
...
**Answer:**
```{r}
my_named_list <- as.list(1:260000)
```
...
```{r}
names(my_named_list) <- paste(letters, rep(1:10000, each=26), sep="")
```
...
```{r}
system.time(
  replicate(100, my_named_list["a5000"])
)
```
```{r}
system.time(
  replicate(100, my_named_list[130001])
)
```

Thus, using a number is faster. Note that the computational costs (speed of your code) will only have to play a role when you are in need for speed. 
...
**Answer:**
```{r}
my_result <- lapply(my_vector, sqrt)
class(my_result)
length(my_result)
```
...
numbers_to_letters <- lapply(my_vector, NumberToLetter)
```
...
```
It has replicated the `list(rnorm(5))` command 10 times, and put the results into a list (of length 10).
...
mean(my_list)
```
No! It does not work, because `mean` requires a vector of numbers (or logicals), not a vector of list elements.
...
**Answer:**
```{r}
write.table(my_data, file = "0_data/my_data.txt")
```
...
```{r}
head(read.table("0_data/my_data.txt", stringsAsFactors = FALSE))
```

`R` apparently now thinks that there are no column names provided, and interprets the first line (with the variable names) as entries of the variables. Because, now, some of the entries are characters, it has to make the mode of the variable character!
...
```{r}
head(read.table("0_data/my_data.txt", header=TRUE))
```
...
```{r}
my_diff_data <- data.frame(
  numeric_variable_one = rnorm(100),
  numeric_variable_two = rnorm(100)
)
write.table(my_diff_data, "0_data/difficult.txt", dec=".", sep=";")
...
```{r}
order(c(3, 1, 2)) # the result is: 2 3 1 e.g. the second element should come first, then the third element, then the first element. This gives us the ordering: 1 2 3
```
...
```{r}
my_data_frame_ordered <- my_data_frame[order(my_data_frame$id), ]
head(my_data_frame_ordered)
```
...
```{r}
my_data_frame <- cbind(my_data_frame, group = rep(1:5, 10))
#  without using cbind:
my_data_frame$group <- rep(1:5, 10)
```
...
**Answer:**
```{r}
idx <- order(my_data_frame$group, my_data_frame$id)
head(my_data_frame[idx, ], 3)
tail(my_data_frame[idx, ], 3)
```
...
```{r}
paste(apply(jumble, 1, function(x) {

...
```{r}
flights[flights$dest == 'SFO' |flights$dest == 'OAK','dest']
flights[flights$arr_delay> 60,'arr_delay']
flights[DFflights$arr_delay> 2*flights$dep_delay ,]
```
...
```{r}
flights[(flights$dest == 'SFO' |flights$dest == 'OAK') & flights$arr_delay> 60 & flights$arr_delay > 2*flights$dep_delay & flights$dep_time<500 ,]
```
...
```{r}
write.csv(data.set, file = "0_data/my_fake_data.csv")
```
